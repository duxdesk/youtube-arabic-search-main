...
const YouTubePlayer = ({ videoId, results = [], currentIndex = 0, totalMatchesCount = 0, startTime, onNavigate }: YouTubePlayerProps) => {
  ...
  // Create the embed URL with timestamp if provided
  const embedUrl = actualVideoId ? `https://www.youtube.com/embed/${actualVideoId}?autoplay=1${startTime !== undefined ? `&start=${Math.floor(startTime)}` : ''}` : '';
